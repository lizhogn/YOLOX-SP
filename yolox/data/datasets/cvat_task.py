...
class CVATTaskDataset(Dataset):
    ...
    def __init__(
        self,
        tasks_dir,  # task_dir or task_list
        imgs_name="train",
        anno_name="annotations.xml",
        img_size=(640, 640),
        preproc=None,
        mosaic=False,
    ...
    def _load_annotations(self):
        ...
        """
        if isinstance(self.tasks_dir, str):
            task_list = [os.path.join(self.tasks_dir, x) for x in os.listdir(self.tasks_dir) 
                            if os.path.isdir(os.path.join(self.tasks_dir, x))]
        else:
            task_list = self.tasks_dir

        annotation_list = []
        for task_id in task_list:
            img_dir = os.path.join(task_id, self.imgs_name)
            anno_path = os.path.join(task_id, self.anno_name)
...
if __name__ == "__main__":
    dataset = CVATTaskDataset(        
        tasks_dir="/home/zhognli/YOLOX/datasets/total/each_task",
        imgs_name="test",
        anno_name="annotations.xml",
        img_size=(640, 640),
